function extractDimensions(geometries) {
  const dimensions = geometries.map(({ vertices }) => {
    const numberOfVertices = vertices.length
    const area = polygonArea(vertices)

    const hull = convexHull(vertices)
    const hullArea = polygonArea(hull)
    const convexityRatio = hullArea === 0 ? 1 : area / hullArea

    const aspectRatio = boundingBoxAspectRatio(vertices)

    const perimeter = polygonPerimeter(vertices)
    const compactness = area === 0 ? 1 : perimeter ** 2 / (4 * Math.PI * area)
