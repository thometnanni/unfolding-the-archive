    ...
    return (p) => {
      ...
      p.draw = () => {
        ...
        if (showOverview) {
          ...
          const minShapePx = 150
          const rawShapeScale = canvasSize / globalMaxDim
          const thisMinScale = minShapePx / Math.max(bounds.w, bounds.h)
          const shapeScale = Math.max(rawShapeScale, thisMinScale)
